...
from control.RegisterMealController import RegisterMealController as Controller
...
class RegisterMealScreen:
    def __init__(self, email):
        ...
        self.carbohydrates = CTkEntry(
            master=self.root, 
            placeholder_text="carboidratos (g)", 
            width=210, height=30
        )
        self.carbohydrates.place(relx=0.5, rely=0.34, anchor=CENTER)
        ...
        register_bttn = CTkButton(
            ...
            font=('Segoe UI', 16),
            width=210, height=30, 
            command=lambda: [self.register_bttn_action(email)]
        )
    ...
    def pick_date(self, event):
        ...
        date_window.title('Escolha a data da refeição')
    ...
    def pick_time(self, event):
        ...
        time_window.title('Escolha o horário da refeição')
    ...
    def register_bttn_action(self, email):
        result = Controller.register_meal(self, email)
        if result == "Refeição registrada com sucesso!":
            sucess = CTkLabel(
                master=self.root,
                text=result,
                text_color="green",
            ...
            error = CTkLabel(
                master=self.root,
                text=f"Erro ao cadastrar refeição: \n{result}",
                text_color="red",
                font=("Segeo UI", 16, "bold")
            )
